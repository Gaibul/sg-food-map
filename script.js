...
function createPopupContent(dataKey, name, food, description, tips, mapLink) {
    ...
    return `
        ${photo ? `<img src="${photo}" width="100%" style="margin-bottom: 5px;" /><br/>` : ""}<b>${name}</b><br/>
...
const popup = createPopupContent(key, Name, Food, Description, Tips && Tips.toLowerCase() !== 'nan' ? Tips : '', GoogleMapsURL);
