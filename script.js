...
// Unified Confirm Save Function
function saveAll(key) {
    const ratingInput = document.getElementById(`rate_${key}`);
    ...
    if (!isNaN(rating)) storedData[key].rating = rating;

    if (noteInput) storedData[key].note = noteInput.value;
...
function createPopupContent(key, name, food, desc, tips, url) {
    ...
    if (rating !== '' && !isNaN(parseFloat(rating))) {
        extraFields = `
            ...
            <br/><br/>
            <button onclick="saveAll('${key}')">✅ Confirm</button>
            ${img}
        `;
    }
    return `
        <div style="max-width:90vw;font-size:14px;">
            <b>${name}</b><br/>
            <i>${food} – ${desc}</i><br/>
            ...
            <input type="number" id="rate_${key}" min="0" max="10" value="${rating}" style="width:100%;" onchange="saveAll('${key}')" />
            ${extraFields}
        </div>
    `;
}
...
// Backup popup only (no extra button)
