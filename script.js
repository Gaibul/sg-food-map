...
function resizeAndCompressImage(file, maxSize = 640, callback) {
    const reader = new FileReader();
...
// Save notes and photo with one Confirm button
document.body.addEventListener('click', function (e) {
    if (e.target.classList.contains('confirmNoteBtn')) {
        ...
        const saveData = (compressedBase64) => {
            storedData[key] = storedData[key] || {};
            storedData[key].note = noteVal;
            if (compressedBase64) storedData[key].photo = compressedBase64;
            localStorage.setItem('foodRatings', JSON.stringify(storedData));
            alert("Notes and photo saved!");
        };

        if (file) {
            resizeAndCompressImage(file, 640, saveData);
        } else {
            saveData(null);
        }
    }
