...
const idbKeyval = (() => {
  ...
  function createStore(dbName, storeName) {
    ...
    dbPromise = promisifyRequest(request).then(db => {
      return {
        getStore(mode) {
  ...
  function get(key) {
    return dbPromise.then(({ getStore }) => promisifyRequest(getStore('readonly').get(key)));
  }
  function set(key, value) {
    return dbPromise.then(({ getStore }) => promisifyRequest(getStore('readwrite').put(value, key)));
  }
  return { createStore, get, set };
})();
idbKeyval.createStore('PhotoDB', 'photos');
...
document.body.addEventListener('click', function (e) {
    if (e.target.classList.contains('confirmNoteBtn')) {
        const key = e.target.dataset.key;
        const noteVal = document.querySelector(`[data-note='${key}']`).value;
        const fileInput = document.querySelector(`[data-img='${key}']`);
        const file = fileInput?.files?.[0];

        storedData[key] = storedData[key] || {};
        storedData[key].note = noteVal;
        localStorage.setItem('foodRatings', JSON.stringify(storedData));
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                img.onload = function () {
                    ...
                    idbKeyval.set(key, compressedData).then(() => {
                        alert("Photo saved!");
                    }).catch(err => alert("Photo save failed: " + err));
                };
...
// Rating fix: persist to localStorage
document.body.addEventListener('change', function (e) {
    if (e.target.classList.contains('ratingInput')) {
        const key = e.target.dataset.key;
        const val = parseFloat(e.target.value);
        storedData[key] = storedData[key] || {};
        storedData[key].rating = val;
        localStorage.setItem('foodRatings', JSON.stringify(storedData));
    }
});

// Load saved image from IndexedDB
function loadPhoto(key, container) {
    idbKeyval.get(key).then(imgData => {
        if (imgData) {
...
document.body.addEventListener('change', function (e) {
    if (e.target.classList.contains('ratingInput')) {
        ...
        storedData[key].rating = val;
        localStorage.setItem('foodRatings', JSON.stringify(storedData));
        // Reload map or update marker color
        location.reload(); // simple fix for now to reflect new color
    }
});
