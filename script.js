...
// Override image upload handling with compression
document.body.addEventListener('change', function (e) {
    if (e.target.classList.contains('photoInput')) {
        const file = e.target.files[0];
        const key = e.target.dataset.key;
        if (file) {
            resizeAndCompressImage(file, 640, function(compressedBase64) {
                storedData[key] = storedData[key] || {};
                storedData[key].photo = compressedBase64;
                localStorage.setItem('foodRatings', JSON.stringify(storedData));
                alert("Image compressed and saved!");
            });
        }
    }
});

// Add confirm/save button logic
